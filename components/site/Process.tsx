...
export function Process() {
  ...
  return (
    <section className="border-y border-hairline bg-card py-20 md:py-28" id="proceso">
          ...
          {steps.map((step, i) => (
            <Reveal
              ...
              className={`flex gap-4 ${
                i === 0 ? "pb-6" : i === steps.length - 1 ? "border-t border-hairline pt-6" : "border-t border-hairline py-6"
              }`}
            >
              <span className="token step-token">{step.n}</span>
              <div>
                <h3 className="text-lg font-medium leading-snug text-white">{step.title}</h3>
